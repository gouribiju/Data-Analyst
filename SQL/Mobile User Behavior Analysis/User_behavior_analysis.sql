...
SELECT ROUND(`Screen On Time (hours/day)`) AS screen_time_group,
...
SELECT 
    CASE 
        WHEN `Number of Apps Installed` <= 20 THEN 'Low (≤20)'
        WHEN `Number of Apps Installed` BETWEEN 21 AND 40 THEN 'Medium (21–40)'
        ELSE 'High (>40)'
    END AS app_count_category,
    ROUND(AVG(`Battery Drain (mAh/day)`), 2) AS avg_battery_drain FROM user_behavior_dataset
GROUP BY app_count_category
ORDER BY avg_battery_drain DESC;
